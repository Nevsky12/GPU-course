...
{
    return liftA1(gltf::round<T>, v);
}

template<typename T>
T fract(T const &x) noexcept
{
    return x - gltf::floor(x);
}
template<typename T>
T mod(T const &x, T const &y) noexcept
{
    return x - y * gltf::floor(x / y);
}
